...
$leadsubsubstatus = Leadsubsubstatu::all(['lead_substatus_id'=>$leadsubstatus[0]->id]);
$leadsub = Leadstatu::all(['id'=>$leadstatus[0]->id]);
$leadst = Leadstype::all(['id'=>$lead_type]);
$leadsubsub = Leadsubstatu::all(['id'=>$leadsubstatus[0]->id]);
//echo $leadcurrentsubstatus;
?>
<div id="page-content-wrapper">
    <div class="container-fluid">
        <div class="profile breadcrumbs" style="padding:5px 10px; font-size:12px">
			<a href="<?=BASEURL?>">Home</a> >> <a href="<?=BASEURL?>administrator/leads">All Leads</a> >> <a href="<?=BASEURL?>administrator/lead/<?=$leadst[0]->name;?>"><?=$leadst[0]->name;?></a> >> <a href="<?=BASEURL?>administrator/leadstatus/<?=$leadst[0]->name;?>/<?=str_replace(' ','_',$leadsub[0]->name); ?>"><?=$leadsub[0]->name;?></a> >> <a href="<?=BASEURL?>administrator/leadsubstatus/<?=$leadst[0]->name;?>/<?=str_replace(' ','_',$leadsub[0]->name); ?>/<?=str_replace(' ','_',$leadcurrentsubstatus) ?>"><?= $leadcurrentsubstatus; ?></a> <?php 
			if(!empty($leadcurrentsubsubstatus)){ echo " >> ".$leadcurrentsubsubstatus;} ?>
			</div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <?php
                        ...
                        foreach( $leadsubsubstatus as $leadst){
                            
                            $labels .= ' "'.$leadst->name.'",';
                            $color = rendomColor()[$i];
                            $labcolor .= ' "'.$color.'",';
                            $leadcounts = Lead::find_by_sql("select COUNT(*) as leadcount from leads where leadfrom = $lead_type and lead_subsubstatus_id = ".$leadst->id." AND status=1");
                        ...
                        $i++;}
                        foreach ($leadsubsubstatus as $key => $value) {

                            echo '<p style="margin-bottom: 0.5em;"><a href="'.BASEURL.'administrator/leadsubstatus/'.$lead_typename.'/'.str_replace(" ","_",$leadcurrentstatus).'/'.str_replace(" ","_",$leadcurrentsubstatus).'/'.str_replace(" ","_",$value->name).'" class="label" style="font-size: 97%;background-color: '.$labelbtnbgcolor[$key].'">'.$value->name.' : '.$leadcount[$key].' leads </a></p>';
...
{
 $leadcountss = $leadcount;   
}
?>
<?php
if($labels=='')
{
$labels12 = '"'.$leadcurrentsubstatus.'"';
}
else
{
$labels12 = $labels;
