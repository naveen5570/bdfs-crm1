...
<div id="page-content-wrapper">
        ...
        <div class="notypnl">
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="activetb">
                    <table class="table table-striped alldatatbll">
                        <thead>
                            <tr>
                                <th>S.No</th>
                                <th>Lead</th>
   ...
   <?php foreach ($list_prev as $prev)
   {
	 ...
	 $user1 ="Select * from subadmins where id=".$prev->assigned_by; 
   ...
   ?>         
                            <tr class="gc_row row<?=$prev->id;?>">
								...
								?>
                                <td><?=$prev->msg;?></td>
                                <td><?= date('m-d-Y', $prev->reminder_date); ?>
                                </td>
								...
								?>
								<td><?= date('m-d-Y', $prev->date); ?></td>
                                <td><?php if($prev->status==0){echo "Pending";} else{ echo "Completed";} ?></td>
								<td><a class="btn btn-info" href="<?=BASEURL?>administrator/editFollowup/<?=$prev->id;?>"><i class="fa fa-pencil"></i></a><a class="btn btn-danger" href="<?=BASEURL?>administrator/deleteFollowup/<?=$prev->id;?>"><i class="fa fa-trash"></i></a></td>
                            </tr>
     <?php
   }
   }
   ...
   ?>         
                            <tr class="gc_row row<?=$today->id;?>">
								...
								?>
                                <td><?=$today->msg;?></td>
                                <td><?= date('m-d-Y', $today->reminder_date); ?>
                                </td>
								...
								?>
								<td><?= date('m-d-Y', $today->date); ?></td>
                                <td><?php if($today->status==0){ echo "Pending";} else{ echo "Completed";} ?></td>
								<td><a href="<?=BASEURL?>administrator/deleteFollowup/<?=$today->id?>">delete</a></td>
                            </tr>
     <?php
   }
}
   ...
   ?>         
                            <tr class="gc_row row<?=$future->id;?>">
								...
								?>
                                <td><?=$future->msg;?></td>
                                <td><?= date('m-d-Y', $future->reminder_date); ?>
                                </td>
								...
								?>
								<td><?= date('m-d-Y', $future->date); ?></td>
                                <td><?php if($future->status==0){ echo "Pending";} else{ echo "Completed";} ?></td>
								<td><a href="<?=BASEURL?>administrator/deleteFollowup/<?=$future->id?>">delete</a></td>
                            </tr>
     <?php
   }
}
